#!/usr/bin/env bash
#
# Self-contained test runner for the network.smtp monitor.
#
...
REQUIREMENTS="$REPO_ROOT/src/network/network.smtp/requirements.txt"

VENV_DIR="$(mktemp -d -t network.smtp-test-XXXXXX)"
...
echo ">> Running network.smtp test cases"
"$VENV_DIR/bin/python" "$HERE/run_smtp_tests.py"
