#!/usr/bin/env bash
#
# Self-contained test runner for the network.ntp monitor.
#
...
REQUIREMENTS="$REPO_ROOT/src/network/network.ntp/requirements.txt"

VENV_DIR="$(mktemp -d -t network.ntp-test-XXXXXX)"
...
echo ">> Running network.ntp test cases"
"$VENV_DIR/bin/python" "$HERE/run_ntp_tests.py"
