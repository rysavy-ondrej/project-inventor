#!/usr/bin/env python3
#
# Auto-generated test harness for src/network/network.snmp (see test/Readme.md).
#
...
MODULE_DIR = os.path.join(REPO_ROOT, "src/network/network.snmp")
INPUT_DIR = os.path.join(HERE, "inputs")
OUTPUT_DIR = os.path.join(HERE, "outputs")
SKIP_HINT = "network.snmp uses easysnmp, which needs system net-snmp headers (e.g. 'sudo apt-get install libsnmp-dev')."

sys.path.insert(0, MODULE_DIR)
try:
    from network_snmp import run
except Exception as e:
    print("SKIPPED: cannot import network_snmp: %s" % e)
    if SKIP_HINT:
        print("         " + SKIP_HINT)
    sys.exit(0)
...
def main():
    ...
    if not inputs:
        ...
        return 1
    print("Running %d network_snmp test case(s)" % len(inputs))
    print("=" * 60)
    failed = skipped = 0
