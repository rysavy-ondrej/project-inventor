#!/usr/bin/env python3
#
# Auto-generated test harness for src/network/network.traceroute (see test/Readme.md).
#
...
MODULE_DIR = os.path.join(REPO_ROOT, "src/network/network.traceroute")
...
try:
    ...
    print("SKIPPED: cannot import network_traceroute: %s" % e)
    if SKIP_HINT:
        print("         " + SKIP_HINT)
    sys.exit(0)
...
def main():
    ...
    if not inputs:
        ...
        return 1
    print("Running %d network_traceroute test case(s)" % len(inputs))
    print("=" * 60)
    failed = skipped = 0
