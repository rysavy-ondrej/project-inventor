#!/usr/bin/env bash
#
# Self-contained test runner for the network.ftp monitor.
#
...
REQUIREMENTS="$REPO_ROOT/src/network/network.ftp/requirements.txt"

VENV_DIR="$(mktemp -d -t network.ftp-test-XXXXXX)"
...
echo ">> Running network.ftp test cases"
"$VENV_DIR/bin/python" "$HERE/run_ftp_tests.py"
