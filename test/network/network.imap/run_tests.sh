#!/usr/bin/env bash
#
# Self-contained test runner for the network.imap monitor.
#
...
REQUIREMENTS="$REPO_ROOT/src/network/network.imap/requirements.txt"

VENV_DIR="$(mktemp -d -t network.imap-test-XXXXXX)"
...
echo ">> Running network.imap test cases"
"$VENV_DIR/bin/python" "$HERE/run_imap_tests.py"
