#!/usr/bin/env bash
#
# Self-contained test runner for the network.mqtt monitor.
#
...
REQUIREMENTS="$REPO_ROOT/src/network/network.mqtt/requirements.txt"

VENV_DIR="$(mktemp -d -t network.mqtt-test-XXXXXX)"
...
echo ">> Running network.mqtt test cases"
"$VENV_DIR/bin/python" "$HERE/run_mqtt_tests.py"
