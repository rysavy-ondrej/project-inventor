#!/usr/bin/env bash
#
# Self-contained test runner for the security.ldap monitor.
#
...
REQUIREMENTS="$REPO_ROOT/src/security/security.ldap/requirements.txt"

VENV_DIR="$(mktemp -d -t security.ldap-test-XXXXXX)"
...
echo ">> Running security.ldap test cases"
"$VENV_DIR/bin/python" "$HERE/run_ldap_tests.py"
