#!/usr/bin/env python3
#
# Auto-generated test harness for src/security/security.ldap (see test/Readme.md).
#
...
MODULE_DIR = os.path.join(REPO_ROOT, "src/security/security.ldap")
...
try:
    ...
    print("SKIPPED: cannot import security_ldap: %s" % e)
    if SKIP_HINT:
        print("         " + SKIP_HINT)
    sys.exit(0)
...
def main():
    ...
    if not inputs:
        ...
        return 1
    print("Running %d security_ldap test case(s)" % len(inputs))
    print("=" * 60)
    failed = skipped = 0
