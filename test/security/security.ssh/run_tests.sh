#!/usr/bin/env bash
#
# Self-contained test runner for the security.ssh monitor.
#
...
REQUIREMENTS="$REPO_ROOT/src/security/security.ssh/requirements.txt"

VENV_DIR="$(mktemp -d -t security.ssh-test-XXXXXX)"
...
echo ">> Running security.ssh test cases"
"$VENV_DIR/bin/python" "$HERE/run_ssh_tests.py"
