#!/usr/bin/env python3
#
# Auto-generated test harness for src/security/security.ssh (see test/Readme.md).
#
...
MODULE_DIR = os.path.join(REPO_ROOT, "src/security/security.ssh")
...
try:
    from security_ssh import run
except Exception as e:
    print("SKIPPED: cannot import security_ssh: %s" % e)
    if SKIP_HINT:
        print("         " + SKIP_HINT)
    sys.exit(0)
...
def main():
    ...
    if not inputs:
        ...
        return 1
    print("Running %d security_ssh test case(s)" % len(inputs))
    print("=" * 60)
    failed = skipped = 0
